...
export function ItemAdministrationHeader({ children, ...restProps }) {
   return <h1 {...restProps}>{children}</h1>;
}
export function ItemAdministrationSubtitle({ children, ...restProps }) {
...
export function ItemAdministration({ children }) {
   ...
   const itemAdministrationHeader = childElements.find(
      (child) => child.type.name === "ItemAdministrationHeader",
   );
   const itemAdministrationSubtitle = childElements.find(
      (child) => child.type.name === "ItemAdministrationSubtitle",
   );
   const itemAdministrationBody = childElements.find(
      (child) => child.type.name === "ItemAdministrationBody",
   );
...
export function ItemAdministrationTable({
   children,
   fallback = null,
   fallbackColumnSpan = "2",
   loading = false,
   ...restProps
}) {
   const childElements = React.Children.toArray(children);
   const tableHeaders = childElements.find(
      (child) => child.type.name === "ItemAdministrationTableHeader",
   );
   const tableBody = childElements.find(
      (child) => child.type.name === "ItemAdministrationTableBody",
   );
...
export function ItemAdministrationRow({ children, ...restProps }) {
   ...
   const itemFragment = childElement.filter(
      (child) =>
         child.type.name === "ItemAdministrationBodyEntry" ||
         child.type.name === "ItemAdministrationHeaderEntry",
   );
   const itemActions = childElement.filter(
      (child) => child.type.name === "ItemAdministrationRowActions",
   );
   return (
      <tr {...restProps}>
         {itemFragment}
